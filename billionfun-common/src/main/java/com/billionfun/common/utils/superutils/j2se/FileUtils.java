...
public class FileUtils {
	...
	public static void copyFileToFold(String sourcePath,String destFold) throws IOException {
		File file = new File(sourcePath);
		if (!(new File(destFold)).exists()) {
			(new File(destFold)).mkdir();
		}
		if (file.isFile()) {
			copyFile(sourcePath, destFold + file.separator + file.getName());
		}
	}
