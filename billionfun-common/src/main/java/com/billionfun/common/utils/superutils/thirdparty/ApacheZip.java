package com.billionfun.common.utils.superutils.thirdparty;
	...
	public static void zipDir(String comments, String targetZip, String dir) {
		...
		try {
			...
			for (File file : files) {
				...
				String entryName = file.getAbsolutePath().replace("\\", systemSeparator).replace("/", systemSeparator).replace(dir.replace("\\", systemSeparator).replace("/", systemSeparator), "");
